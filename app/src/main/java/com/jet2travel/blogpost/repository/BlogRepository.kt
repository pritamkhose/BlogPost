...
open class BlogRepository {
    ...
    fun getBlogList(
        ...
        if (databaseCache.getAllBlogs().isEmpty()) {
            ...
            if (ConnectivityUtils.isNetworkAvailable(context)) {
                ...
                val vbody = databaseCache.getAllBlogs()
                Log.d(Constants.APP_TAG, "get data response from database: $vbody")

                data.value = Resource.success(
                    getDBToBlog(vbody),
                    context.resources.getString(R.string.cached)
                )
            }
        }
        return data
            ...
            override fun onResponse(
                ...
                try {
                    if (response.body()?.size!! > 0) {
                        ...
                        databaseCache.insert(getBlogToDB(body.toMutableList()))
    ...
    private fun getBlogToDB(body: MutableList<Blog>): MutableList<com.jet2travel.blogpost.database.model.Blog> {
        val rows = mutableListOf<com.jet2travel.blogpost.database.model.Blog>()
        val vbody = body.toMutableList()
        vbody.forEach { blog ->
            val userString = gson.toJson(blog.user)
            val mediaString = gson.toJson(blog.media)
            rows.add(
    ...
    private fun getDBToBlog(vbody: MutableList<com.jet2travel.blogpost.database.model.Blog>): MutableList<Blog> {
        val rows = mutableListOf<Blog>()
        vbody.forEach { blog ->
            try {
                val user: Array<User> =
                    gson.fromJson(blog.user, Array<User>::class.java)
                val media: Array<Media> =
                    gson.fromJson(blog.media, Array<Media>::class.java)
                rows.add(
                    Blog(
                        blog.comments,
                        blog.content,
